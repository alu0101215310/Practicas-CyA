...
// Modificación: Contar los símbolos utilizados del alfabeto
void operation::difSymbols() {
  for (int j = 0; j < language.size(); j++) {
    std::set<char> counter;
    for (int i = 0; i < language[j].getChain().size(); i++) {
      auto pos = alphabets[j].getSymbols().find(language[j].getChain()[i]);
      if (pos != alphabets[j].getSymbols().end()) counter.insert(language[j].getChain()[i]);
    }
    output << counter.size() << "/" << alphabets[j].getSymbols().size() << std::endl;
  }
}
